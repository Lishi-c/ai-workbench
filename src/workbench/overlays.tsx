...
function EditorModal({ modal, close }: { modal: Exclude<ModalState, null>; close: () => void }) {
  ...
  const restoreData = async (file?: File) => {
    ...
    try {
      ...
      if (!window.confirm("将用备份覆盖当前所有数据，确定恢复吗？")) return;
      updateData(() => normalizeWorkbenchData(parsed));
      notify("已从备份恢复全部数据");
